<template>
  <div class="sign">
    ...
    />
    <div class="sign-box">
      <form name="signform" method="post" @submit.prevent="sign">
        <h1>账密注册</h1>
        <input type="text" placeholder="账号/邮箱/手机号" v-model="username" />
        <input type="password" placeholder="密码" v-model="password" />
        <button type="submit" @click.prevent="sign">确定</button>
      </form>
      <div class="others">
        注册/登录即表示你同意
        ...
        <a @click="goTo('/login')" class="right">登录用户</a>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    sign() {
      if (this.username === "" || this.password === "") {
        ...
        this.axios.post("/createUser", params).then(res => {
          if (res.data.ret == 0) {
            this.$message.success(`${res.data.msg}`);
            this.axios.post("/loginCheck", params).then(response => {
              this.$store.commit("userInfo", response.data.wdata);
