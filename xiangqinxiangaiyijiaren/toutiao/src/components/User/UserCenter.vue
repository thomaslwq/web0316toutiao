...
export default {
    ...
    methods: {
        ...
        remove({nid,type}){
            ...
            this.$axios.post('/deleteArticle',{nid,oauth_token}).then(res =>{
                if(res.ret)return;
                if(type ==='TT'){
                    this.updateTTCount(--this.userinfo.tt_count)
                    this.ttData.forEach((e,i) =>e.nid == nid && this.ttData.splice(i,i))
                }else{
                    this.updateArticleCount(--this.userinfo.article_count)
                    this.wzData.forEach((e,i) =>e.nid == nid && this.ttData.splice(i,i))
                }
                    this.$message({
                    message:'删除成功',
                    type:'success'
